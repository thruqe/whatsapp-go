...
func handleUpdateCommand(ctx *dispatch.Context) error {
	if !ctx.IsSudo() {
		return ctx.Reply("You are not authorized to use this command.")
	}

	s, _ := dispatch.GetStore(ctx)
...
func handleUpgradeCommand(ctx *dispatch.Context) error {
	if !ctx.IsSudo() {
		return ctx.Reply("You are not authorized to use this command.")
	}
...
func showUpdateStatus(ctx *dispatch.Context, channel string) error {
...
func performCheck(ctx *dispatch.Context, channel string) error {
...
func performUpgrade(ctx *dispatch.Context, isBeta bool) error {
...
func handleChannels(ctx *dispatch.Context) error {
	...
	for i, n := range newsletters {
		...
		if name == "" {
			name = dispatch.Sprintf("Channel %d", i+1)
		}
		...
		tb.Numbered(i+1, dispatch.Bold(name)).
			Indent(3, dispatch.Sprintf("Role: %s | Followers: %d", role, subs)).NewLine().
			Indent(3, dispatch.Sprintf("Link: %s", link)).NewLine().
			Blank()
	}
