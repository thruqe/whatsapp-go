...
// NewTextWithSender initializes a TextBuilder bound to a Sender for direct reply/send dispatch.
func NewTextWithSender(sender Sender, initial ...string) *TextBuilder {
	b := &TextBuilder{sender: sender}
	if len(initial) > 0 {
		b.Text(initial...)
	}
	return b
...
// NewTextWithContext initializes a TextBuilder bound to a Sender (alias for NewTextWithSender).
func NewTextWithContext(sender Sender, initial ...string) *TextBuilder {
	return NewTextWithSender(sender, initial...)
}
...
func (b *TextBuilder) Reply() error {
	if b.sender == nil {
		return fmt.Errorf("TextBuilder: Reply called without Sender")
	}
	text := b.Trimmed()
	if len(b.mentions) > 0 {
		return b.sender.ReplyWithMentions(text, b.mentions)
	}
	return b.sender.Reply(text)
}
...
func (b *TextBuilder) Send() error {
	if b.sender == nil {
		return fmt.Errorf("TextBuilder: Send called without Sender")
	}
	text := b.Trimmed()
	if len(b.mentions) > 0 {
		return b.sender.SendTextWithMentions(text, b.mentions)
	}
	return b.sender.SendText(text)
}
...
func (b *TextBuilder) ReplyWithImage(data []byte, mimetype string) error {
	if b.sender == nil {
		return fmt.Errorf("TextBuilder: ReplyWithImage called without Sender")
	}
	caption := b.Trimmed()
	if len(b.mentions) > 0 {
		return b.sender.ReplyWithImageWithMentions(data, mimetype, caption, b.mentions)
	}
	return b.sender.ReplyWithImage(data, mimetype, caption)
}
...
func (b *TextBuilder) SendWithImage(data []byte, mimetype string) error {
	if b.sender == nil {
		return fmt.Errorf("TextBuilder: SendWithImage called without Sender")
	}
	caption := b.Trimmed()
	if len(b.mentions) > 0 {
		return b.sender.SendImageWithMentions(data, mimetype, caption, b.mentions)
	}
	return b.sender.SendImage(data, mimetype, caption)
}
